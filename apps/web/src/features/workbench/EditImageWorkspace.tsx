...
import { editErrorLabel, editExecutionModeLabel, editOperationImpactHint, editOperationLabel } from "../../lib/userText";
...
export function EditImageWorkspace({ projectId, project, output, outputs, assets, onSelectOutput, onClose }: { projectId: string; project?: EditProjectDefaults; output: Output | undefined; outputs: Output[]; assets: Asset[]; onSelectOutput: (outputId: string) => void; onClose: () => void }) {
  ...
  const lastOutputRef = useRef<Output | undefined>(undefined);
  const prevEditKeyRef = useRef<string | null>(null);
  useEffect(() => { if (output) lastOutputRef.current = output; }, [output]);
  // 弹窗关闭时保留编辑状态：以最后一次选中的成图继续渲染，重开同一张图不重置会话与画布
  const activeOutput = output ?? lastOutputRef.current;
  ...
  const ensureSession = async () => { if (sessionId) return sessionId; if (!activeOutput) throw new Error("未选择成图"); const response = await fetch(`${API_BASE_URL}/projects/${projectId}/outputs/${activeOutput.id}/edit-sessions`, { method: "POST" }); if (!response.ok) throw new Error(await response.text()); const data = await response.json() as EditSessionState; setSessionId(data.id); setSession(data); return data.id; };
  const selectVersion = async (versionId: string) => {
    if (!session || !activeOutput || versionId === activeOutput.id) return;
  ...
  const saveMemory = async () => { if (!session || !activeOutput) return; const constraints = memoryConstraints.split(/\r?\n/).map((value) => value.trim()).filter(Boolean); const response = await fetch(`${API_BASE_URL}/edit-sessions/${session.id}/memory`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ outputId: activeOutput.id, summary: memorySummary.trim(), constraints }) }); if (!response.ok) throw new Error(await response.text()); const updated = await response.json() as EditSessionState; setSession((current) => current ? { ...current, ...updated } : updated); setMemorySourceOutputId(updated.memorySummary?.sourceOutputId); };
  ...
  return <Modal open={Boolean(output)} onCancel={onClose} footer={null} width="min(1380px, calc(100vw - 32px))" className={styles.editModal} title="编辑图片" zIndex={1100}>
    {activeOutput ? <div className={styles.editWorkspace}>
      ...
      <aside className={styles.editAgentPanel}>
        ...
        {sessionOutputs.length > 0 ? <div className={styles.editVersions}><p>连续版本</p><div>{sessionOutputs.map((version, index) => { const baseLabel = versionLabel(version); const duplicateCount = sessionOutputs.filter((candidate) => versionLabel(candidate) === baseLabel).length; const ordinal = sessionOutputs.slice(0, index).filter((candidate) => versionLabel(candidate) === baseLabel).length + 1; return <div key={version.id} className={styles.editVersionRow}><button type="button" data-current={activeOutput.id === version.id} onClick={() => void selectVersion(version.id)}><span>{baseLabel}{duplicateCount > 1 ? ` · ${ordinal}` : ""}</span><small>{activeOutput.id === version.id ? "当前" : "历史"}</small></button>{index < sessionOutputs.length - 1 && activeOutput.id !== version.id ? <button type="button" onClick={() => setCompareOutputId(compareOutputId === version.id ? null : version.id)}>{compareOutputId === version.id ? "取消对比" : "对比当前"}</button> : null}</div>; })}</div></div> : null}
        {compareOutput && compareOutput.id !== activeOutput.id ? <div className={styles.editCompare}><div><img src={activeOutput.url} alt="当前版本" /><span>当前</span></div><div><img src={compareOutput.url} alt="对比版本" /><span>对比</span></div></div> : null}
        ...
        {turn?.plan?.userSummary ? <div className={styles.editPlan}><p>{turn.plan.userSummary}</p><span>{editOperationLabel(turn.plan.operation)}</span>{turn.plan.executionMode ? <small>执行方式：{editExecutionModeLabel(turn.plan.executionMode)}</small> : null}{turn.plan.targetDescription ? <small>目标：{turn.plan.targetDescription}{typeof turn.plan.targetConfidence === "number" ? ` · 置信度 ${Math.round(turn.plan.targetConfidence * 100)}%` : ""}</small> : null}{editOperationImpactHint(turn.plan.operation) ? <small>影响范围：{editOperationImpactHint(turn.plan.operation)}</small> : null}</div> : null}
