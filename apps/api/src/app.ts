...
export async function buildApi(options: ApiOptions): Promise<FastifyInstance> {
  ...
  app.post("/api/v1/jobs/:jobId/retry", async (request, reply) => { const id = parameter(request, "jobId"); const job = repository.getJob(id); if (!job) missing("job", id); if (!job.retryable) throw new ApiError(409, "CONFLICT", "This job cannot be retried"); const input = job.type === "GENERATE" ? { ...job.input, revision: "retry" } : job.input;
