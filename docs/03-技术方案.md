...
模型能力由 Provider 配置显式描述：`supportsVision`、`supportsThinking`、`supportsTools`、`supportsStructuredOutput`、`imageApiKind`。推理模型不支持视觉时，系统只根据用户填写内容和已选图片类型规划，并将该限制回传前端；不得静默切模型。
