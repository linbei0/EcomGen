...
export const SegmentationModelRef = Type.Object({
  ...
  modelId: Type.String(),
  protocol: Type.Optional(Type.Union([Type.Literal("fal"), Type.Literal("grounded_sam"), Type.Literal("seedream_layerize")], { description: "Segmentation API protocol; derived from the model's declared segmentationProtocol and re-stated here for the Worker adapter. A body protocol that contradicts the declared one is rejected." }))
...
export const ModelCapability = Type.Object({ id: Type.String(), supportsVision: Type.Boolean(), supportsThinking: Type.Boolean(), supportsTools: Type.Boolean(), supportsStructuredOutput: Type.Boolean(), imageApiKind: Type.Optional(Type.Union([Type.Literal("openai_images"), Type.Literal("gemini"), Type.Literal("custom"), Type.Null()])), segmentationProtocol: Type.Optional(Type.Union([Type.Literal("fal"), Type.Literal("grounded_sam"), Type.Literal("seedream_layerize")], { description: "Segmentation API protocol declared for this model (fal.ai SAM 3, self-hosted Grounded-SAM, or Volcengine Seedream layer decomposition); mutually exclusive with imageApiKind." })) }, { $id: "#/components/schemas/ModelCapability" });
...
export const LayerPlanElement = Type.Object({ id: Type.String({ description: "Stable element ID inside the plan; manual elements reuse the client-provided ID." }), name: Type.String({ description: "Editable element display name." }), source: Type.Union([Type.Literal("auto"), Type.Literal("manual")]), bbox: Type.Optional(Type.Union([schemaRef(LayerBbox), Type.Null()])) }, { $id: "#/components/schemas/LayerPlanElement" });
