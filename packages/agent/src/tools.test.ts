...
describe("Pi planning business tools", () => {
  ...
  it("keeps visual research disabled by default and constrains enabled search output", async () => {
    expect(createPlanningTools(["AMAZON"])).toHaveLength(2);
    ...
    try {
      const tool = createPlanningTools(["AMAZON"], { sources: [{ id: "brave", name: "Brave", kind: "brave", baseUrl: "https://search.example.test", apiKey: "secret" }], maxResults: 1, audit: { onSearchStarted: ({ query }) => started.push(query), onSourceAttempt: (attempt) => attempts.push(attempt) } })[2]!;
  ...
  it("uses the next configured source after the preferred source fails", async () => {
    ...
    try {
      const tool = createPlanningTools(["AMAZON"], { sources: [
