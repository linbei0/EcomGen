...
describe("Pi planning business tools", () => {
  ...
  it("fails explicitly for unknown templates and scopes platform guidance to project context", async () => {
    ...
    expect(details.copyPolicy).toBeUndefined();
    expect(details.planningPolicy).toBeUndefined();
  });
  ...
  it("derives the market language only when the user did not select one", async () => {
    ...
    const result = await tool.execute("call-market", {});
    const guidance = result.details as { market: { id: string; visualDirection?: unknown }; effectiveCopyLanguage: string; copyLanguageSource: string; product: { family: string | null } };
  ...
  it("uses the next configured source after the preferred source fails", async () => {
    ...
    try {
      const tool = createPlanningTools(amazonContext, {
        sources: [
        ...
        ], audit: { onSearchStarted: () => { }, onSourceAttempt: (attempt) => attempts.push(attempt) }
      })[2]!;
